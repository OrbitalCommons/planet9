...
class IrasAkari2025(Scene):
    def construct(self):
        ...
        l1 = layout.label("IRAS 1983 (60 µm)", font_size=16, color=P.RED).next_to(p1, DOWN, buff=0.2)
        self.play(FadeIn(p1), FadeIn(l1))
        p2 = Dot(LEFT * 2.0 + RIGHT * 1.6 + UP * 0.25, radius=0.10, color=P.ORANGE)
        l2 = layout.label("AKARI 2006 (90 µm)", font_size=16, color=P.ORANGE).next_to(p2, UP, buff=0.2)
        arr = Arrow(p1.get_center(), p2.get_center(), color=P.TEAL, buff=0.12, stroke_width=3)
        self.play(FadeIn(p2), FadeIn(l2), Create(arr))
        mv = layout.label("proper motion over 23 yr matches a ~500–700 AU body",
