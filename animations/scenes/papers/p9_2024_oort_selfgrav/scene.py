...
"""
import numpy as np
from manim import Create, DOWN, FadeIn, Scene, Text, UP, VGroup, Write, rate_functions, ValueTracker, always_redraw
...
class OortSelfgrav2024(Scene):
    def construct(self):
        ...
        live = always_redraw(lambda: VGroup(*[
            ...
            for v in varpis0]))
        self.add(live)
        self.add(Text("collective self-gravity → coherent precession", font_size=17, color=P.TEAL).to_edge(UP, buff=1.5))
        self.play(drift.animate.set_value(np.pi), run_time=4.0, rate_func=rate_functions.linear)
        self.play(FadeIn(layout.takeaway("The disk can stir itself -- another way to get coherent apsidal motion.")))
        self.wait(0.9)
