...
"""
import numpy as np
from manim import Create, DOWN, FadeIn, FadeOut, Line, Scene, UP, UR, Write
...
class CowanThermal2016(Scene):
    def construct(self):
        ...
        self.play(Create(ax), FadeIn(layout.label("wavelength (µm, log)", font_size=18, color=P.FG).next_to(ax, DOWN, buff=0.25)))
        for lx, t in [(0, "1"), (1, "10"), (2, "100"), (3, "1000")]:
            self.add(layout.label(t, font_size=13, color=P.MUTED).next_to(ax.c2p(lx, 0), DOWN, buff=0.1))
        ...
        for wl, name, col in [(60, "IRAS", P.RED), (90, "AKARI", P.ORANGE), (1000, "mm", P.PURPLE)]:
            self.add(layout.label(name, font_size=14, color=col).move_to(ax.c2p(np.log10(wl), 0.9)))
