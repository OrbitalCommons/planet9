...
class DesIsotropy2020(Scene):
    def construct(self):
        ...
        varpis = rng.uniform(0, 2 * np.pi, 9)  # DES sample looks ~isotropic
        ...
        self.play(Write(MathTex(rf"\bar{{R}} = {rbar:.2f}\ \text{{(no strong signal)}}", color=P.RED).scale(0.6).to_corner(UR, buff=0.4)))
