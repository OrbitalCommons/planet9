...
import numpy as np
from manim import (
    Create, DOWN, FadeIn, Scene, UP, Write,
)
...
class DetachedInclinations2021(Scene):
    def construct(self):
        ...
        ax = widgets.axes([0, 40, 10], [0, 1.05, 0.5], x_length=9.3, y_length=3.9, font_size=18, shift_down=0.4)
        xl = layout.label("inclination (deg)", font_size=18, color=P.FG).next_to(ax, DOWN, buff=0.2)
        yl = layout.label("relative number", font_size=16, color=P.FG).next_to(ax, UP, buff=0.05)
        ...
        self.add(layout.label("no Planet Nine", font_size=15, color=P.MUTED).next_to(ax.c2p(6, 1.0), UP, buff=0.05))
        self.play(Create(with_p9))
        wlbl = layout.label("with Planet Nine", font_size=15, color=P.GREEN).next_to(ax.c2p(22, gauss(22, 18, 9)), UP, buff=0.1)
