...
"""
import numpy as np
from manim import Create, DOWN, FadeIn, Line, RIGHT, Scene, UP, VGroup, Write
...
class Commensurabilities2016(Scene):
    def construct(self):
        ...
        self.play(Create(ax), FadeIn(layout.label("distance from nearest simple period ratio", font_size=17, color=P.FG).next_to(ax, DOWN, buff=0.25)))
        ...
        elbl = layout.label("excess near commensurability", font_size=15, color=P.PURPLE).next_to(ax.c2p(0.2, 0.84), UP, buff=0.05)
