...
class Perturbation2025(Scene):
    def construct(self):
        ...
        self.play(Create(ax),
                  ...
                  FadeIn(layout.label("survival probability", font_size=15, color=P.FG).next_to(ax, UP, buff=0.05)))
        curve = ax.plot(lambda a: 1.0 / (1.0 + np.exp(-(a - 420) / 50.0)), x_range=[200, 800, 5], color=P.TEAL)
        self.play(Create(curve), run_time=1.5)
        albl = layout.label("analytic boundary matches N-body", font_size=16, color=P.TEAL).next_to(ax, UP, buff=0.4)
