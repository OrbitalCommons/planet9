...
import numpy as np
from manim import (
    Create, DOWN, FadeIn, Indicate, RIGHT, Scene, UP, VGroup, Write,
)
...
class NewDiscoveries2025(Scene):
    def construct(self):
        ...
        klab = layout.label("known clustered ETNOs", font_size=15, color=P.GREEN).to_edge(UP, buff=1.5)
        ...
        alab = layout.label("2017 OF201 (fits)", font_size=14, color=P.TEAL).to_edge(DOWN, buff=1.7)
        self.play(FadeIn(alab))
        timing.hold_to_read(self, alab, settle=0.5)
        self.play(Create(new_off))
        olab = layout.label("Ammonite (off the cluster)", font_size=14, color=P.ORANGE).to_edge(DOWN, buff=1.3)
